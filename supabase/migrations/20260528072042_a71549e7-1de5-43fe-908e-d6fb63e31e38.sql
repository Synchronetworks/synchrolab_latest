ALTER TABLE public.courses
  ...
  ADD COLUMN IF NOT EXISTS early_bird_until date NULL,
  ADD COLUMN IF NOT EXISTS sibling_price numeric NULL;

CREATE OR REPLACE FUNCTION public.create_booking(_type booking_type, _customer_name text, _email text, _phone text, _num_pax integer, _total_amount numeric, _course_id uuid DEFAULT NULL::uuid, _slot_id uuid DEFAULT NULL::uuid, _room_id uuid DEFAULT NULL::uuid, _booking_date_from date DEFAULT NULL::date, _booking_date_to date DEFAULT NULL::date, _company text DEFAULT NULL::text, _notes text DEFAULT NULL::text, _promo_code text DEFAULT NULL::text)
 RETURNS text
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  IF _type = 'course'::booking_type AND _course_id IS NOT NULL THEN
    ...
    IF FOUND THEN
      ...
      END IF;
      IF _num_pax >= 2 AND v_course.sibling_price IS NOT NULL AND v_course.sibling_price < v_unit THEN
